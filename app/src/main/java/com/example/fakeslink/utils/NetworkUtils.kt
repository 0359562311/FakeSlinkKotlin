package com.example.fakeslink.utils
...
object ErrorInterceptor : Interceptor {
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (response.code == 401
            && chain.request().url.pathSegments.contains("auth/jwt").not()
            && GlobalVariable.session != null
        ) {
            val sessionService = NetworkUtils.buildService(SessionService::class.java)
